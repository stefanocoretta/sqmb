...
**How can a probability generate binary outcomes?**
  ...
  - If it's not a fair coin, you might get 9 heads and 1 tail ($p = 0.9$).
]
...
- There is a 95% probability that the true change in log-odds is between –0.3 and +0.67.
- To figure out the **probability of answering correctly**, we'll put some of this math to use.
]
...
```{r plog-2, echo=TRUE}
# Same strategy to figure out lower bound of 95% CrI:
...
## How does a model estimate 95% CrIs?
...
95% of the distribution is between the 2.5th centile and the 97.5th centile.
]
...
- We model the binary outcomes as being generated the probability of success $p$.
...
- They are the area in which, with some probability, the true value lies.
]
