...
There is another type of file paths, called **relative** paths. A relative path is a partial file path, relative to a specific folder. You will learn how to use relative paths below, when we will go through importing files in R using R scripts.
...
And since the `read_csv()` is a function from the tidyverse, you first need to attach the tidyverse with `library(tidyverse)` (remember, you need to attach packages **only once** per session).

So, putting all together, here's the code. **Remember to change the file path to match the path of the file on your computer**.
...
**Measures of dispersion** (standard deviation, variance, range) indicate the dispersion of the sample values around the central tendency value.
:::
...
We have covered this in the lecture, so we won't go over it again here. Instead, you will learn how to obtain summary measures using the `summarise()` function.
...
We can use the `count()` function to count the number of occurrences for each value of a specific column. Let's count how many trials are correct, i.e. let's count occurrences in the `ACC` column.
...
You can use the `group_by()` function, together with `summarise()` to achieve that. Let's see how it works.
...
The pipe `%>%` teleports the output of the preceding function as the first argument of the following function.
The output of `group_by` is a (groupped) tibble, and `summarise()` needs a tibble as its first argument.
...
Don't worry too much if the concept of the pipe is not clear yet.
