...
TODO: Get draws and plot cond probs density (and from this, get 95% CrIs).
...
shallow %>% 
  ...
  labs(y = 'Proportion',
       x = 'Relation type') +
  # Add in the model's estimates as points + bars
...
We have already seen that the coefficient for VAR2 is VAR2_COEF at the baseline level of VAR1, and so we can understand the interaction as saying that the importance of VAR2 as a predictor [*increases* if signs of VAR2_COEF and INTERACTION_COEF are the same/*decreases* if signs of VAR2_COEF and INTERACTION_COEF are different(?)] when we move from VAR1_BASELINE to VAR2_BASELINE.

```{r}
# var1 <- 
```

TODO: Compare prose interpretation from sqmf w7.
...
TODO: Overlay these estimates on the actual data.

TODO: On same slide, show model predictions from the first model (no interaction) and compare to those from the second model (with interaction).
