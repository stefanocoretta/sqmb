...
This tutorial has two sections, and each one will guide you through a different example of interactions between two binary variables.
...
We've worked with the data from `shallow.csv` before, first way back in week 2 (ah, the good old days!).
...
Try to understand the conditions that `filter()` is applying to the dataset.
...
There is a column called `ACC` that contains 0s and 1s, though, so below, we'll use that to create the `Accuracy` column of our dreams.
...
Create a bar plot that looks like this one, showing the proportion of correct/incorrect responses by `Relation_type`, faceting by `Branching`.
...
- To include the text "Branching" in the facet labels, give the function `facet_grid()` the following argument: `labeller = labeller(Branching = label_both)`. 


Notice that this plot shows a _difference of differences_: The difference between `Relation_type == Unrelated` and `Relation_Type == Constituent` is bigger when `Branching == Left`, and smaller when `Branching == Right`.
...
With treatment coding like this, the $\beta$ coefficient tells us the effect of moving from the baseline level to the non-baseline level.
In this case, the baseline is `Unrelated`.
This means that the $\beta$ coefficient will represent the estimated effect of moving from `Unrelated` to `Constituent`.

In other words:

- The $\beta$ coefficient's 95% CrI will be mostly or entirely **positive** if greater accuracy is associated with `Relation_type == Constituent` than with `Relation_type == Unrelated`.
- The $\beta$ coefficient's 95% CrI will be mostly or entirely **negative** if lower accuracy is associated with `Relation_type == Constituent` than with `Relation_type == Unrelated`.
- The $\beta$ coefficient's 95% CrI will be **centered around zero** if there is no clear association between accuracy and relation type.

Based on this information and on the plot above, do you expect that the coefficient's posterior distribution will be largely positive, negative, or around zero?
...
Here, the baseline is `Left`, so $\beta$ will represent the estimated effect of moving from `Left` to `Right`.

In other words:

- The $\beta$ coefficient's 95% CrI will be mostly or entirely **positive** if greater accuracy is associated with `Branching == Right` than with `Branching == Left`.
- The $\beta$ coefficient's 95% CrI will be mostly or entirely **negative** if lower accuracy is associated with `Branching == Right` than with `Branching == Left`.
- The $\beta$ coefficient's 95% CrI will be **centered around zero** if there is no clear association between accuracy and branching.
...
We're now going to create the conditional posterior probability distributions of the probability of answering correctly in each combination of `Relation_type` and `Branching`.
We'll then talk about how to interpret and report the model's estimates based on these distributions.
...
(Fun fact: these are the samples that `summary()` summarises when it shows us the model's estimates.)
...
To know how to combine these columns in order to get the correct conditional posterior probability distributions, we have to reason about how the 0/1 treatment coding affects the presence or absence of the $\beta$s.
...
(Note that, in `acc_draws`, the column name for the interaction has `:` in it, which can be confusing to R.
To avoid problems, we wrap that column name in tick marks.)
...
acc_draws_cond_long <- acc_draws_cond %>% 
  pivot_longer(
    # The columns to pivot (here, it's all of them):
...
- We're going to use a function from the tidyverse called `separate()` to split the `predictors` column up into two columns: one for `Relation_type`, and another for `Branching`. Have a look at [the documentation](https://tidyr.tidyverse.org/reference/separate.html) if you're curious.
- This is a new data frame, so R will apply its defaults for factor ordering unless we again set the factor levels manually.
...
**Note:** When computing the conditional posterior probability distributions, it is very important to do any addition/subtraction **in the space the model was fit in**.
That is, figuring out the conditional probability distribution must be done **before** back-transforming into the original space.
Otherwise, if you, say, back-transform each individual $\beta$ and then add them together, you risk ending up with incorrect or even impossible values on your original scale.
:::

#### Compute means and 95% CrIs

We'll also use the long draws data to compute the summary statistics of the posteriors.
...
I mentioned above that the posterior draws are what `summary()` uses to summarise the posterior distributions.
So, a good sense check to make sure our computation is good is to compare `acc_cond_summ` with the actual model summary.
In the model summary, the estimates for `Intercept` should match the summary measures here for the row in which `Relation_type == Unrelated` and `Branching == Left` (since those are the two baseline levels).
...
So now we can be reassured that we've computed something sensible and worth reporting.


#### Plot multiple posterior densities

Now we'll use the conditional posterior probability distributions to create a density plot.
...
That's it for our first model!
Now we'll turn to the second part of the tutorial: a slightly more loosely guided walkthrough of how to put what you've learned into practice.


## Vowel duration in Italian and Polish

The data we'll use for the rest of this tutorial comes from Coretta, S., (2019) "An exploratory study of voicing-related differences in vowel duration as compensatory temporal adjustment in Italian and Polish", *Glossa: a journal of general linguistics* 4(1): 125. doi: <https://doi.org/10.5334/gjgl.869>.

The dataset can be downloaded here: [dur-ita-pol.csv](../data/dur-ita-pol.csv).

### Read in the data

Save `dur-ita-pol.csv` in your `data/` folder, and read it in using read it in using `read_csv()`.
...
This section will consider what kinds of transformations and level re-orderings, if any, we'll need to do to prepare the data for analysis.
We'll begin with the outcome variable, `v1_duration`.


#### Vowel duration

Let's create a density plot to visualise how vowel duration is distributed.
...
- The distribution is slightly right-skewed (that is, it has a longer tail going off to the right).
...
1. Apply the transformation you've chosen to the `v1_duration` variable.
2. Make a new density plot to see whether your transformation has worked.

*(For real! Pause here and work through it yourself first. The strongest learning comes by doing.)*
...
OK, let's now have a look at how the categorical predictors stack up against vowel duration.
...
These conditional posterior probability distributions are going to be over the log vowel duration, so the unit will be in log(ms).
Similar to how we back-transformed the log-odds into probabilities above using `plogis()`, you'll want to back-transform the log(ms) into ms by using the inverse of the `log()` function, which is `exp()`.

Reproduce the workflow we went through above.
This will involve:
...
- We use interactions to tell the model that the effect of one predictor can be different within levels of another predictor.

- We can use our knowledge of treatment coding to predict the sign (i.e., positive or negative) of $\beta$ coefficients, even before the model is fit.


**Data processing and visualisation**

- `separate()` can split up a single column into multiple ones, as long as every value in that column contains a separator, e.g., `_`.

- The same way that we back-transform log-odds into probabilities using `plogis()`, we back-transform log values into the original space using `exp()`.

- All addition/subtraction to create conditional posterior probability distributions must be done on the scale in which the model was fit (e.g., log-odds or log), not the original space (e.g., probability or ms).
