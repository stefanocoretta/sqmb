...
\text{var = 1 :}   & & \beta_0 &+ (\beta_1 \cdot 1) &&= \beta_0 + \beta_2\\
...
## Let's build a model predicting vowel duration<br> by speech rate
...
**Centering continuous variables is pretty much always a good idea.**
...
- The mean log vowel duration when speech rate is at its mean of 0 and when the vowel is "a"<br> (coded as 0) is 4.79 (95% CrI: [4.77, 4.81]).
...
- When the vowel is "a" (coded as 0), the change in the mean log vowel duration <br> when speech rate increases by one syllable/second is –0.26 (95% CrI: [–0.29, –0.24]).
...
- When the centered speech rate is at its mean of 0, the change in the mean log vowel duration <br> when changing from the vowel "a" to the vowel "o" is –0.04 (95% CrI: [–0.07, –0.01]).
...
.bg-washed-blue.b--dark-blue.ba.bw2.br3.shadow-5.ph4.mt2[
**`speech_rate_c:vowelo`, interpretation 1:**

- When we move from the vowel "a" (baseline) to "o" (non-baseline), there is a positive adjustment <br> (0.04 log ms, 95% CrI [0, 0.07]) to the effect of speech rate.

]

.bg-washed-blue.b--dark-blue.ba.bw2.br3.shadow-5.ph4.mt2[
**`speech_rate_c:vowelo`, interpretation 2:**

- When we increase the speech rate by one syllable/second, there is a positive adjustment <br> (0.04 log ms, 95% CrI [0, 0.07]) to the effect of the vowel.
]
...
- **The slope (effect) when vowel = "a" is more strongly negative than "o".**
]
...
With continuous variables, choose a few representative values to compute <br>conditional posterior distributions for. 
For centered speech rate, e.g.,
...
- They're already numeric, so they *can* go into the model as-is.
...
- To compute conditional posterior probability distributions, it's useful to **choose a few values<br> from the continuous predictor** to evaluate (e.g., the mean and on either side of it).
]
...
## Topics this course has not covered 

**(but has hopefully equipped you to tackle yourselves, if you want!)**

- How to set and interpret group-level effects (a.k.a., varying effects, random effects)
  - Adjustments to intercept and slope for, e.g., every participant.
- How to select and set your own priors (we've been using the brms defaults).
- Contrast coding beyond 0/1 treatment coding.
- The Bayesian version of hypothesis testing using Bayes Factors.
