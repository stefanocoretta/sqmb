...
In the lecture, we have seen that we can assume voiceless VOT values to be distributed according to a Gaussian distribution.
...
In other words, we are trying to estimate the following parameters: $\mu_1$, $\sigma_1$, $\mu_2$, $\sigma_2$.
...
Add `library(brm)` in the `setup` chunk and run that chunk. Then, run the following code.
...
Based on the model summary and on the tidy summary, we also know the **95% Credible Intervals** (CIs or CrIs) of the probability distributions of $\mu$ and $sigma$.
...
Another way is to visualise probability distributions by plotting probability densities. You will learn how to do this next week.
...
-   The `%in%` operator returns `TRUE` if the value to the left is one of the values in the list to its right, and `FALSE` if not.
