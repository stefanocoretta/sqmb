...
If you compare the output of your model and the output shown here, hou might notice that the values for the estimates and error are different.
...
Now, this is well and good, but often it's easier to think about our results in terms of probabilities, rather than log-odds.
So what do we do if we want to plot the **probability** of choosing "takete"?
Before looking ahead, try to guess for yourself what the solution will be.
...
The distributions themselves look pretty similar, but have a look at the numbers on the x axis.
We're back in probability space, hurrah!
