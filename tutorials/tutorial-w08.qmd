...
Because we are using 0/1 treatment coding, the $\beta$ coefficient tells us the effect of moving from the baseline level to the non-baseline level.
...
**In general, for treatment-coded predictors, a positive interaction coefficient represents a positive adjustment to one variable's effect, when we move from (or in other words, when we compare) the other variable's baseline level to its non-baseline level.
...
- there is a smaller effect of `Relation_type` in `Branching == Right` (non-baseline) when compared to `Branching == Left` (baseline); 
...
The 95% CrI of $[-1.72, 0.42]$ suggests that the model thinks that a negative interaction is most probable (there is more probability mass on the negative side of zero than the positive side).
However, since positive values also come into question, this suggests that the model is slightly uncertain about the direction of the interaction (but it definitely thinks a negative interaction is more probable).
...
We'll also use `acc_draws_cond_long` to compute summary statistics of the posteriors.
...
Because `summary()` also summarises with these posterior draws, a good sense check is to make sure `acc_cond_summ` matches the actual model summary.
Specifically, in the model summary, the estimates for `Intercept` should match the summary measures here for the row in which `Relation_type == Unrelated` and `Branching == Left` (since those are the two baseline levels).
...
Now we'll turn to the second part of the tutorial: a more loosely-guided walkthrough of how to put what you've learned so far into practice.
