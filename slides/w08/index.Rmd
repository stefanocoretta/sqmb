...
## What will the betas mean now?

- Show linear expression of mult model
- Fill in vars with 0s and 1s, showing which betas are left over when the 0s cancel out terms
...
## Treatment coding matrix + an interaction

- start with treatment coding matrix from earlier
- create new var for interaction as product [be hand-wavey about why this works, but can recommend lit]
- work through 0/1 algebra to show which ebtas are left
- highlight the new thing: now in this one case, we have an extra coefficient, the interaction, that can modulate the existing effects that we estimate for the other two predictors
  - contrast the math from prev non-interaction version and this one — the final line is different
